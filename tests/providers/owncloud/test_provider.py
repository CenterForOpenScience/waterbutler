...
class TestValidatePath:
    ...
    async def test_validate_v1_path_file(self, provider, file_metadata):
        ...
        try:
            wb_path_v1 = await provider.validate_v1_path('/myfile.txt')
        except Exception as exc:
            pytest.fail(str(exc))

        wb_path_v0 = await provider.validate_path('/myfile.txt')
...
class TestCRUD:
    ...
    async def test_download(self, provider, file_metadata, file_like):
        ...
        aiohttpretty.register_uri('GET', url, body=b'squares', auto_length=True)
