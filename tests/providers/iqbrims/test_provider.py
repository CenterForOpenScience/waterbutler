...
from waterbutler.providers.iqbrims import settings as ds
from waterbutler.providers.iqbrims import IQBRIMSProvider
...
def provider(auth, credentials, settings):
    return IQBRIMSProvider(auth, credentials, settings)


@pytest.fixture
def other_provider(auth, other_credentials, settings):
    return IQBRIMSProvider(auth, other_credentials, settings)
...
class TestValidatePath:
    ...
    async def test_validate_v1_path_root(self, provider):
        ...
        expected = IQBRIMSPath('/', _ids=[provider.folder['id']], folder=True)
    ...
    async def test_revalidate_path_file(self, provider, root_provider_fixtures):
        ...
        path = IQBRIMSPath(file_name, _ids=['0', file_id])
...
class TestMetadata:
    ...
    * revision parameter: non, valid, invalid, and magic
    ...
    async def test_metadata_file_root(self, provider, root_provider_fixtures):
        ...
        expected = IQBRIMSFileMetadata(file_metadata, path)
        assert result == expected
    ...
    async def test_folder_metadata(self, provider, root_provider_fixtures):
        ...
        expected = IQBRIMSFolderMetadata(item, path.child(item['title'], folder=True))
