...
class TestValidatePath:
    ...
    def test_root(self, provider):
        path = yield from provider.validate_v1_path('/')
        assert path.path == ''
    ...
    @async
    def test_package(self, provider):
        stripped_doi = provider.doi.split('.')[-1]
        path = yield from provider.validate_v1_path('/'+stripped_doi+'/')
    ...
    @async
    def test_file(self, provider):
        stripped_doi = provider.doi.split('.')[-1]
        path = yield from provider.validate_path('/'+stripped_doi+"/1")
        assert path.name == '1'
    ...
    def test_validate_v1_path_package(self, provider, object_package_metadata):
        ...
        stripped_doi = provider.doi.split('.')[-1]
        wb_path_v1 = yield from provider.validate_v1_path('/' + stripped_doi + '/')

        with pytest.raises(exceptions.NotFoundError) as exc:
            yield from provider.validate_v1_path('/XXXX')
        assert exc.value.code == client.NOT_FOUND

        wb_path_v0 = yield from provider.validate_path('/' + stripped_doi + '/')

        assert wb_path_v1 == wb_path_v0

    @async
    @pytest.mark.aiohttpretty
    def test_validate_v1_path_file(self, provider, object_file_metadata):
        ...
        stripped_doi = provider.doi.split('.')[-1]
        wb_path_v1 = yield from provider.validate_v1_path('/' + stripped_doi + "/1")

        with pytest.raises(exceptions.NotFoundError) as exc:
            yield from provider.validate_v1_path('/' + stripped_doi + "/9999")
        assert exc.value.code == client.NOT_FOUND

        wb_path_v0 = yield from provider.validate_path('/' + stripped_doi + '/1')
...
class TestMetadata:
    ...
    @async
    @pytest.mark.aiohttpretty
    def test_metadata_root(self, provider, object_package_metadata):
        ...
        root_metadata = yield from provider.metadata('/')

    @async
    @pytest.mark.aiohttpretty
    def test_metadata_package(self, provider,
        ...
        aiohttpretty.register_uri('GET', DRYAD_META_URL + provider.doi.split('.')[-1]+"/1",
                              ...
                              headers={'Content-Type': 'application/xml'})
        aiohttpretty.register_uri('GET', DYRAD_FILE_URL + provider.doi.split('.')[-1]+"/1/bitstream",
        ...
        stripped_doi = provider.doi.split('.')[-1]
        path = yield from provider.validate_v1_path('/' + stripped_doi + '/')
        package_metadata = yield from provider.metadata(path)
    ...
    def test_metadata_file(self, provider,
        ...
        aiohttpretty.register_uri('GET', DRYAD_META_URL+ provider.doi.split('.')[-1]+"/1/bitstream",
                              ...
                              headers={'Content-Type': 'application/xml',
                                        'CONTENT-DISPOSITION': 'attachment; filename="myfile.txt'})
        stripped_doi = provider.doi.split('.')[-1]
        path = yield from provider.validate_v1_path('/' + stripped_doi + '/1')
        file_metadata = yield from provider.metadata(path)
