...
class TestUpload:
    ...
    @pytest.mark.asyncio
    @pytest.mark.aiohttpretty
    async def test_upload_create_chunked_upload_session(self, provider, root_provider_fixtures):
        """Check that the chunked upload session creation makes a request to
        the correct address.
        """
        path = WaterButlerPath('/newfile', _ids=(provider.folder, None))
        ...
        session_data = await provider._create_chunked_upload_session(path, b'DATA')
    ...
    async def test_upload_parts(self, provider, root_provider_fixtures):
        ...
        parts_metadata = await provider._upload_chunks('tenbytestr'.encode() * 2, session_metadata)
        ...
        for call in aiohttpretty.calls:
            ...
            assert call['uri'] == provider._build_upload_url(
                'files',
                'upload_sessions',
                'fake_session_id'
            )
    ...
    async def test_complete_chunked_upload_session(self, provider, root_provider_fixtures):
        commit_url = provider._build_upload_url(
            'files',
            'upload_sessions',
            'fake_session_id',
            'commit'
