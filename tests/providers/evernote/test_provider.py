...
from waterbutler.providers.evernote import EvernoteProvider
...
from waterbutler.providers.evernote.settings import EVERNOTE_META_URL, EVERNOTE_FILE_URL
...
@pytest.fixture
def settings():
    return {'doi': 'doi:10.5061/evernote.1850'}
...
def object_package_metadata():
    return """
        <EvernoteDataPackage xmlns="http://purl.org/evernote/schema/terms/v3.1"
            xmlns:dwc="http://rs.tdwg.org/dwc/terms/"
            xmlns:dcterms="http://purl.org/dc/terms/"
            xmlns:bibo="http://purl.org/evernote/schema/evernote-bibo/v3.1"
            xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
            xsi:schemaLocation="http://purl.org/evernote/schema/terms/v3.1
                http://dataevernote.org/profile/v3.1/evernote.xsd">
          ...
          <dcterms:identifier>http://dx.doi.org/10.5061/evernote.1850</dcterms:identifier>
          <dcterms:description>
          ...
          <dcterms:hasPart>http://dx.doi.org/10.5061/evernote.1850/1</dcterms:hasPart>
        </EvernoteDataPackage>
    """
...
def object_file_metadata():
    return """
        <EvernoteDataFile xmlns="http://purl.org/evernote/schema/terms/v3.1"
            xmlns:dwc="http://rs.tdwg.org/dwc/terms/"
            xmlns:dcterms="http://purl.org/dc/terms/"
            xmlns:bibo="http://purl.org/evernote/schema/evernote-bibo/v3.1"
            xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
          ...
          <dcterms:identifier>
            http://dx.doi.org/10.5061/evernote.1850/1
          </dcterms:identifier>
          <dcterms:rights>
          ...
          <dcterms:isPartOf>http://dx.doi.org/10.5061/evernote.1850</dcterms:isPartOf>
        </EvernoteDataFile>
"""
...
def meta_file_metadata():
    ...
    <d1:systemMetadata xsi:schemaLocation="http://ns.dataone.org/service/types/v1
      ...
      <identifier>doi:10.5061/evernote.1850/1/bitstream</identifier>
      ...
      <rightsHolder>admin@dataevernote.org</rightsHolder>
      <accessPolicy>
        <allow>
      ...
      <originMemberNode>urn:node:EVERNOTE</originMemberNode>
      <authoritativeMemberNode>urn:node:EVERNOTE</authoritativeMemberNode>
    </d1:systemMetadata>
    """
...
class TestValidatePath:
    ...
    async def test_validate_v1_path_package(self, provider, object_package_metadata):
        aiohttpretty.register_uri(
            'GET', EVERNOTE_META_URL + provider.doi.split('.')[-1],
            body=object_package_metadata, headers={'Content-Type': 'application/xml'}
        )
        aiohttpretty.register_uri(
            'GET', EVERNOTE_META_URL + 'XXXX',
            status=404
        )
    ...
    async def test_validate_v1_path_file(self, provider, object_file_metadata):
        ...
        aiohttpretty.register_uri(
            'GET', EVERNOTE_META_URL +  provider.doi.split('.')[-1] + "/1",
            body=object_file_metadata, headers={'Content-Type': 'application/xml'}
        )
        aiohttpretty.register_uri(
            'GET', EVERNOTE_META_URL +  provider.doi.split('.')[-1] + "/9999",
            status=404
        )
        stripped_doi = provider.doi.split('.')[-1]
...
class TestMetadata:
    ...
    async def test_metadata_root(self, provider, object_package_metadata):
        ...
        aiohttpretty.register_uri('GET', EVERNOTE_META_URL +provider.doi.split('.')[-1],
                                  body=object_package_metadata,
                                  headers={'Content-Type': 'application/xml'})

        path = WaterButlerPath('/')
        root_metadata = await provider.metadata('/')
    ...
    async def test_metadata_package(self, provider,
        ...
        aiohttpretty.register_uri('GET', EVERNOTE_META_URL  + provider.doi.split('.')[-1]+'/',
                              body=object_package_metadata,
                              headers={'Content-Type': 'application/xml'})
        aiohttpretty.register_uri('GET', EVERNOTE_META_URL + provider.doi.split('.')[-1]+"/1",
                              body=object_file_metadata,
                              headers={'Content-Type': 'application/xml'})
        aiohttpretty.register_uri('GET', EVERNOTE_FILE_URL + provider.doi.split('.')[-1]+"/1/bitstream",
                              body=meta_file_metadata,
                              headers={'Content-Type': 'application/xml'})
        aiohttpretty.register_uri('GET', EVERNOTE_META_URL + provider.doi.split('.')[-1]+"/1/bitstream",
                              body=file_content,
                              headers={'Content-Type': 'application/xml',
