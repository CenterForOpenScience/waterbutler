...
class MetadataMixin:
    ...
    async def download_folder_as_zip(self):
        zipfile_name = f'{self.path_kwargs.get("resource")}-{self.provider.NAME}-{self.path.name}' if self.path.name else f'{self.path_kwargs.get("resource")}-{self.provider.NAME}-archive'
