...
from waterbutler.providers.iqbrims import utils


class BaseIQBRIMSMetadata(metadata.BaseMetadata):
    ...
    @property
    def extra(self):
        """NB: The IQB-RIMS provider frequently fudges this value in the raw data before passing
        it to the IQBRIMS*Metadata constructor.  The ``version`` field provided by the GDrive
        ...
        IQBRIMSProvider docstring."""

        return {'revisionId': self.raw['version']}


class IQBRIMSFolderMetadata(BaseIQBRIMSMetadata, metadata.BaseFolderMetadata):
...
class IQBRIMSFileRevisionMetadata(IQBRIMSFileMetadata):
    """The metadata for a single file at a particular revision on IQB-RIMS.  This class expects
    ...
    derive it from the `IQBRIMSPath` object.
...
class IQBRIMSRevision(metadata.BaseFileRevisionMetadata):
