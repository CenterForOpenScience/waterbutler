...
from waterbutler.providers.googlecloudstorage.metadata import (GoogleCloudStorageMetaData,
                                                               GoogleCloudStorageFileMetadata,
                                                               GoogleCloudStorageFolderMetadata,)
...
class GoogleCloudStorageProvider(BaseProvider):
    ...
    async def metadata(
            ...
            **kwargs
    ) -> typing.Union[GoogleCloudStorageMetaData, typing.List[GoogleCloudStorageMetaData]]:
        ...
        :rtype GoogleCloudStorageMetaData: for file
        :rtype List<GoogleCloudStorageMetaData>: for folder
        """
    ...
    async def _metadata_object(
        ...
        """Get the RAW metadata about the object with the given Waterbutler path.
        ...
        :rtype GoogleCloudStorageMetaData
        """
        ...
        if is_folder:
            return GoogleCloudStorageFolderMetadata(dict(data))
        else:
            return GoogleCloudStorageFileMetadata(dict(data))

    async def _metadata_folder(
            self,
            path: WaterButlerPath
    ) -> typing.List[GoogleCloudStorageMetaData]:
        ...
        Need set the object name of the folder as the prefix, and use '/' as the delimiter.  Here is
        an example request: "https://www.googleapis.com/storage/v1/b/gcloud-test.longzechen.com/o
        ?prefix=test-folder-1%2F&delimiter=%2F".  Below is an example of the json response. "items"
        contains the folder itself and all its immediate child files.  "prefixes" contains all its
        immediate chile folders.

        {
            "kind": "storage#objects",
            "prefixes": [
                "test-folder-1/test-folder-5/",
                "test-folder-1/test-folder-6/"
            ],
            "items": [
                {
                    ...,
                ...
                {
                    ...,
                    "name": "test-folder-1/DSC_0244.JPG",
                    ...,
                }
            ]
        }
        ...
        :rtype List<GoogleCloudStorageMetaData>
        """
        ...
        for item in data.get('items', []):
            if item.get('name', '') != prefix:
                folder_metadata_list.append(GoogleCloudStorageFileMetadata(dict(item)))
    ...
    async def create_folder(
            ...
            path: WaterButlerPath,
            **kwargs
    ) -> typing.Tuple[GoogleCloudStorageMetaData, bool]:
        ...
        Create a folder in Google Cloud Storage is basically to upload an object of which the name
        ends with a `/`.  If the folder exists, it will be replaced with the new folder and all its
        ...
        return GoogleCloudStorageFolderMetadata(dict(data)), created
    ...
    async def upload(
        ...
        return GoogleCloudStorageFileMetadata(dict(data)), created
    ...
    async def download(
        ...
        Note: download request on folders return HTTP 200 OK with empty body.

        :param path: the Waterbutler path to the object to download
        :param range: an optional tuple (start, end) that is transformed into a Range header
        :param kwargs: additional kwargs are ignored
    ...
    async def delete(self, path: WaterButlerPath, *args, **kwargs) -> None:
        ...
        Docs: https://cloud.google.com/storage/docs/json_api/v1/objects/delete
    ...
    async def _delete_folder(self, path: WaterButlerPath) -> None:
        ...
        Docs: https://cloud.google.com/storage/docs/json_api/v1/how-tos/batch
        Note: the limitations and work-around apply for all batch requests: copy and delete
    ...
    async def _batch_delete_items(
        ...
        """Make a batch request that delete multiple objects on Google Cloud Storage.  Returns a
        list of id of failed requests.
        ...
        3. TODO: add timeout or threshold
    ...
    async def intra_copy(
        ...
        """Copy objects within the Google Cloud Storage Provider.

        For files, issue a single "copyTo" or "rewriteTo" request and return the target's metadata.
        For folders, first issue a metadata request to retrieve all the children, and then issue a
        batch "copyTo" or "rewriteTo" request to copy all of them (including the folder itself).

        Please note that Google Cloud Storage provides two ways to copy objects.  Currently, use
        "copyTo" since the response is the metadata of the destination file.
        Docs for "rewriteTo": https://cloud.google.com/storage/docs/json_api/v1/objects/rewrite
        Docs for "copyTo": https://cloud.google.com/storage/docs/json_api/v1/objects/copy
    ...
    async def _intra_copy_file(
        ...
        return GoogleCloudStorageFileMetadata(data), created
    ...
    async def _intra_copy_folder(
        ...
        """ Copy folders and all its content within the same provider.  Similar to deleting folders,
        use the batch request. Please refer to ``_delete_folder`` for limitations and work-around.

        The destination folder and all its contents must be deleted before the copy starts.
        """
    ...
    async def _batch_copy_items(
        ...
        """Make a batch request that copy multiple objects on Google Cloud Storage.  Returns a list
        of metadata for successful ones.
    ...
    def can_intra_copy(self, other: BaseProvider, path: WaterButlerPath=None) -> bool:
        """Google Cloud Storage supports intra copy for both file and folders.
    ...
    def can_duplicate_names(self):
        """Google Cloud Storage allows a file and a folder shares the same name.
        """
    ...
    def build_payload_for_batch_delete(self, items: list, id_prefix: str) -> typing.Tuple[str, dict]:
